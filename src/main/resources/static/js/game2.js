...
letshapeRotateMap = [1,0,3,2,4,6,7,8,5,10,9,12,13,14,11,16,17,18,15];
let shapeColorArray = [
    "rgb(199,82,82)",
    "rgb(233,174,43)",
    "rgb(105,155,55)",
...
const tileColor = "white", wallColor = "black";
let shapeColor;
let fastMode = false;
const initSpeed = 400,
    deltaSpeed = 40,
...
function gebi(y,x){
    const ret = document.getElementById(String(y)+" "+String(x));
    return ret;
}
...
function drawField() {
    let fieldTag = "<table id=\"gameTable\">";
    for(let i=0;i<H;i++){
        fieldTag += "<tr>";
        for(let j=0;j<W;j++)
...
function gameOver(){
    ...
    if(localStorage.getItem("score")) {
        ...
        const newScore = parseInt(userScore) + score;
